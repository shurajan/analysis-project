...
pub enum ParseError {
    ...
    InvalidNumber,
    /// Ноль недопустим в данном контексте
    ZeroValue,
...
mod stdp { // parsers for std types
    ...
    impl Parser for I32 {
        ...
        fn parse<'a>(&self, input: &'a str) -> Result<(&'a str, Self::Dest), ParseError> {
            let end_idx = input.char_indices().skip(1)
                               ...
                               .unwrap_or(input.len());
            let value = input[..end_idx].parse().map_err(|_| ParseError::InvalidNumber)?;
            if value == 0 {
                return Err(ParseError::ZeroValue); // в наших логах нет нулей, ноль в операции - фикция
            }
    ...
    impl Parser for Byte {
        type Dest = u8;
        fn parse<'a>(&self, input: &'a str) -> Result<(&'a str, Self::Dest), ParseError> {
            let (to_parse, remaining) = input.split_at_checked(2).ok_or(ParseError::UnexpectedEnd)?;
            if !to_parse.chars().all(|c| c.is_ascii_hexdigit()) {
                return Err(ParseError::InvalidChar);
            }
            let value = u8::from_str_radix(to_parse, 16).map_err(|_| ParseError::InvalidNumber)?;
            Ok((remaining, value))
        }
    }
}
...
fn do_unquote<'a>(input: &'a str) -> Result<(&'a str, String), ParseError> {
    ...
    let mut chars = input.strip_prefix("\"").ok_or(ParseError::ExpectedTag)?.chars();
    ...
    Err(ParseError::UnclosedQuote) // строка кончилась, не закрыв кавычку
}
...
impl Parser for Tag {
    type Dest = ();
    fn parse<'a>(&self, input: &'a str) -> Result<(&'a str, Self::Dest), ParseError> {
        Ok((input.strip_prefix(self.tag).ok_or(ParseError::ExpectedTag)?, ()))
    }
}
/// Конструктор [Tag]
        ...
        match self.parsers.0.parse(input) {
            ...
            Err(_)
                => match self.parsers.1.parse(input) {
                    ...
                    Err(_) => self.parsers.2.parse(input).and_then(
...
impl<T: Parser> Parser for List<T> {
    ...
    fn parse<'a>(&self, input: &'a str) -> Result<(&'a str, Self::Dest), ParseError> {
        let mut remaining = input.trim_start().strip_prefix('[').ok_or(ParseError::ExpectedTag)?.trim_start();
        ...
        while !remaining.is_empty() {
            match remaining.strip_prefix(']') {
                ...
                None => {
                    ...
                    let new_remaining = new_remaining.trim_start().strip_prefix(',').ok_or(ParseError::ExpectedTag)?.trim_start();
                    result.push(item);
                    remaining = new_remaining;
                },
            }
        }
        Err(ParseError::UnclosedBracket) // строка кончилась, не закрыв скобку
    }
}
...
impl Parsable for AuthData {
    ...
    fn parser () -> Self::Parser {
        map(take(AUTHDATA_SIZE, stdp::Byte), |authdata| AuthData(Box::new(authdata.try_into().unwrap_or([0;AUTHDATA_SIZE]))))
    }
}
...
impl Parsable for Backet {
    ...
    fn parser () -> Self::Parser {
        map(delimited(
            ...
            strip_whitespace(tag("}"))
        ), |(asset_id, count)| Backet { asset_id, count: count.get() })
    }
}
...
impl Parsable for UserCash {
    ...
    fn parser () -> Self::Parser {
        map(delimited(
            ...
            strip_whitespace(tag("}"))
        ), |(user_id, count)| UserCash { user_id, count: count.get() })
    }
}
...
impl Parsable for LogLine {
    ...
    fn parser () -> Self::Parser {
        map(
            ...
            |(kind, request_id)| LogLine { kind, request_id: request_id.get() }
        )
    }
}
...
impl LogLineParser {
    pub fn parse<'a>(&self, input: &'a str) -> Result<(&'a str, LogLine), ParseError> {
...
mod test {
    ...
    fn test_u32() {
        ...
        assert_eq!(stdp::U32.parse(""), Err(ParseError::InvalidNumber));
        assert_eq!(stdp::U32.parse("-3"), Err(ParseError::InvalidNumber));
        assert_eq!(stdp::U32.parse("0x03"), Ok(("", nzu(0x3))));
        assert_eq!(stdp::U32.parse("0x03abg"), Ok(("g", nzu(0x3ab))));
        assert_eq!(stdp::U32.parse("0x"), Err(ParseError::InvalidNumber));
    }
    ...
    fn test_i32() {
        ...
        assert_eq!(stdp::I32.parse(""), Err(ParseError::InvalidNumber));
        assert_eq!(stdp::I32.parse("-3"), Ok(("", -3)));
        assert_eq!(stdp::I32.parse("0x03"), Err(ParseError::ZeroValue));
        assert_eq!(stdp::I32.parse("-"), Err(ParseError::InvalidNumber));
    }
    ...
    fn test_do_unquote_non_escaped() {
        ...
        assert_eq!(do_unquote_non_escaped(r#" "411""#), Err(ParseError::ExpectedTag));
        assert_eq!(do_unquote_non_escaped(r#"411"#), Err(ParseError::ExpectedTag));
    }
    ...
    fn test_unquote() {
        ...
        assert_eq!(Unquote.parse(r#" "411""#), Err(ParseError::ExpectedTag));
        assert_eq!(Unquote.parse(r#"411"#), Err(ParseError::ExpectedTag));
    ...
    fn test_tag() {
        ...
        assert_eq!(tag("key=").parse("key:value"), Err(ParseError::ExpectedTag));
    }

    #[test]
    fn test_quoted_tag() {
        assert_eq!(quoted_tag("key").parse(r#""key"=value"#), Ok(("=value", ())));
        assert_eq!(quoted_tag("key").parse(r#""key:"value"#), Err(ParseError::ExpectedTag));
        assert_eq!(quoted_tag("key").parse(r#"key=value"#), Err(ParseError::ExpectedTag));
    }
    ...
    fn test_strip_whitespace() {
        ...
        assert_eq!(strip_whitespace(stdp::U32).parse(" 42 answer"), Ok(("answer", nzu(42))));
    }

    #[test]
    fn test_delimited() {
        assert_eq!(delimited(tag("["), stdp::U32, tag("]")).parse("[0x32]"), Ok(("", nzu(0x32))));
        assert_eq!(delimited(tag("["), stdp::U32, tag("]")).parse("[0x32] nice"), Ok((" nice", nzu(0x32))));
        assert_eq!(delimited(tag("["), stdp::U32, tag("]")).parse("0x32]"), Err(ParseError::ExpectedTag));
        assert_eq!(delimited(tag("["), stdp::U32, tag("]")).parse("[0x32"), Err(ParseError::ExpectedTag));
    }

    #[test]
    fn test_key_value() {
        assert_eq!(key_value("key", stdp::U32).parse(r#""key":32,"#), Ok(("", nzu(32))));
        assert_eq!(key_value("key", stdp::U32).parse(r#"key:32,"#), Err(ParseError::ExpectedTag));
        assert_eq!(key_value("key", stdp::U32).parse(r#""key":32"#), Err(ParseError::ExpectedTag));
        assert_eq!(key_value("key", stdp::U32).parse(r#" "key" : 32 , nice"#), Ok(("nice", nzu(32))));
    }

    #[test]
    fn test_list() {
        assert_eq!(list(stdp::U32).parse("[1,2,3,4,]"), Ok(("", vec![nzu(1), nzu(2), nzu(3), nzu(4)])));
        assert_eq!(list(stdp::U32).parse(" [ 1 , 2 , 3 , 4 , ] nice"), Ok(("nice", vec![nzu(1), nzu(2), nzu(3), nzu(4)])));
        assert_eq!(list(stdp::U32).parse("1,2,3,4,"), Err(ParseError::ExpectedTag));
    ...
    #[test]
    fn test_asset_dsc() {
        assert_eq!(all2(strip_whitespace(tag("AssetDsc")),strip_whitespace(tag("{"))).parse(" AssetDsc { "), Ok(("", ((),()))));
    ...
    #[test]
    fn test_log_kind() {
        assert_eq!(preceded(strip_whitespace(tag("NetworkError")), strip_whitespace(unquote())).parse(r#"NetworkError "url unknown""#), Ok(("", "url unknown".into())));

        assert_eq!(LogKind::parser().parse(r#"System::Error NetworkError "url unknown""#), Ok(("", LogKind::System(SystemLogKind::Error(SystemLogErrorKind::NetworkError("url unknown".into()))))));
        ...
        assert_eq!(LogKind::parser().parse(r#"App::Journal CreateUser {"user_id": "Steeve", "authorized_capital": 10000,}"#), Ok(("", LogKind::App(AppLogKind::Journal(AppLogJournalKind::CreateUser{user_id: "Steeve".into(), authorized_capital: 10_000})))));
        assert_eq!(LogKind::parser().parse(r#"App::Journal DeleteUser {"user_id": "Steeve",}"#), Ok(("", LogKind::App(AppLogKind::Journal(AppLogJournalKind::DeleteUser{user_id: "Steeve".into()})))));
        ...
        assert_eq!(LogKind::parser().parse(r#"App::Journal DepositCash UserCash{"user_id": "Steeve", "count": 10,}"#), Ok(("", LogKind::App(AppLogKind::Journal(AppLogJournalKind::DepositCash(UserCash{user_id: "Steeve".into(), count: 10}))))));
