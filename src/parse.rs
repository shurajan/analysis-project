    ...
    impl Parser for Byte {
        ...
        fn parse<'a>(&self, input: &'a str) -> Result<(&'a str, Self::Dest), ParseError> {
            let (to_parse, remaining) =
                input.split_at_checked(2).ok_or(ParseError::UnexpectedEnd)?;
...
impl Parsable for AuthData {
    ...
    fn parser() -> Self::Parser {
        map(take(AUTHDATA_SIZE, stdp::Byte), |authdata| {
            AuthData(Box::new(authdata.try_into().unwrap_or([0; AUTHDATA_SIZE])))
        })
    }
}
